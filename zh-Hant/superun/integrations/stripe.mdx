...
**聊天驅動的自動設置（推薦）**
連接 **superun 雲服務**（或 Supabase）,並在 **研發 → 服務 → Stripe** 中設定好 Stripe 憑證（如下文的設定步驟）,然後只需在聊天中描述你的需求:
...
- 對於**一次性銷售**,請確保您的購物車或產品頁面已正常運行.
  - 對於 **訂閱**,請確認 superun 雲服務（或 Supabase）身份驗證已到位,以便 superun 可以將 Stripe 客戶鏈接到每個用戶的 **id**
</Note>
...
* 在 **瀏覽器控制台 → 網絡/錯誤**,**superun 雲服務 → 日誌** 和 **Stripe 儀表板 → 日誌** 中進行調試.
...
* 項目**必須**連接到 **superun 雲服務**（或 Supabase）. [詳細了解 superun 雲服務](/zh-Hant/superun/integrations/cloud) 或 [Supabase 集成](/zh-Hant/superun/integrations/supabase)
...
superun 現在為您生成所有 Stripe 邏輯.當你透過 **研發 → 服務 → Stripe** 設定好 **STRIPE_API_KEY**（以及可選的 **STRIPE_WEBHOOK_SECRET**）並且專案已**連接到 superun 雲服務**（或 Supabase）後,只需在聊天中告訴 superun 你需要什麼,無需手動編碼.

<Steps>
  <Step title="第 1 步:準備您的項目">
    * **superun 雲服務** （或 Supabase）已連接
...
*"為我的'數字課程'創建一次性結賬,價格為 29 美元"
  ...
  <Step title="第 3 步:審核併申請">
    superun 自動搭建支付組件,數據庫表（如果使用 superun 雲服務）和 UI 元素（全部與用戶 ID 相關聯）的支架.檢查預覽,然後單擊**應用**進行部署.
  </Step>
  </Steps>

<Note>
  * 訂閱應始終鏈接到 superun 雲服務（或 Supabase）中經過身份驗證的用戶的“id”,以實現安全,基於角色的訪問.
</Note>
...
## 高級集成:Webhooks 和 superun 雲服務

對於復雜的支付結構,例如**訂閱和基於角色的訪問**,superun 建議使用 **superun 雲服務**（或 Supabase）來安全地處理 Stripe 集成.這允許正確的 Webhook 處理,訂閱管理和基於支付級別的基於角色的訪問控制.
...
<Steps>
  <Step title="第 1 步:連接 superun 雲服務">
    入門很簡單. superun 使連接 superun 雲服務 變得毫不費力:

1. 轉到 **研發 → 服務**
    2. 找到 **superun 雲服務** 並點擊 **啟用**
    3. 按照說明設置您的雲項目
    4. 連接後,superun 雲服務 可實現安全支付處理,訂閱管理,Webhook 處理,客戶數據存儲和錯誤處理.
  </Step>

  <Step title="第 2 步:檢索 Webhook 端點 URL">
    superun 生成 Webhook 處理程序後,從以下位置檢索端點 URL:
    * **superun 雲服務** → **函數** → 您的 webhook 函數
  ...
  <Step title="第 4 步:安全存儲 Webhook 密鑰">
    將 Webhook 簽名密鑰存儲在您的 superun 雲服務 環境變量中,或者在聊天中詢問 superun 以幫助您安全地配置它.
...
<Accordion title="查看 superun 雲服務 日誌">
    <Steps>
        <Step title="第 1 步">
        轉到 **研發 → 服務 → superun 雲服務**
      </Step>
