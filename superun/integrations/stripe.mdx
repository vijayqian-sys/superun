...
**聊天驱动的自动设置（推荐）**
连接 **superun 云服务**（或 Supabase）,并在 **研发 → 服务 → Stripe** 中设定好 Stripe 凭证（如下文的设定步骤）,然后只需在聊天中描述你的需求:
...
- 对于**一次性销售**,请确保您的购物车或产品页面已正常运行.
  - 对于 **订阅**,请确认 superun 云服务（或 Supabase）身份验证已到位,以便 superun 可以将 Stripe 客户链接到每个用户的 **id**
</Note>
...
* 在 **浏览器控制台 → 网络/错误**,**superun 云服务 → 日志** 和 **Stripe 仪表板 → 日志** 中进行调试.
...
* 项目**必须**连接到 **superun 云服务**（或 Supabase）. [详细了解 superun 云服务](/superun/integrations/cloud) 或 [Supabase 集成](/superun/integrations/supabase)
...
superun 现在为您生成所有 Stripe 逻辑.当你通过 **研发 → 服务 → Stripe** 设定好 **STRIPE_API_KEY**（以及可选的 **STRIPE_WEBHOOK_SECRET**）并且项目已**连接到 superun 云服务**（或 Supabase）后,只需在聊天中告诉 superun 你需要什么,无需手动编码.

<Steps>
  <Step title="第 1 步:准备您的项目">
    * **superun 云服务** （或 Supabase）已连接
...
*"为我的'数字课程'创建一次性结账,价格为 29 美元"
  ...
  <Step title="第 3 步:审核併申请">
    superun 自动搭建支付组件,数据库表（如果使用 superun 云服务）和 UI 元素（全部与用户 ID 相关联）的支架.检查预览,然后单击**应用**进行部署.
  </Step>
  </Steps>

<Note>
  * 订阅应始终链接到 superun 云服务（或 Supabase）中经过身份验证的用户的“id”,以实现安全,基于角色的访问.
</Note>
...
## 高级集成:Webhooks 和 superun 云服务

对于复杂的支付结构,例如**订阅和基于角色的访问**,superun 建议使用 **superun 云服务**（或 Supabase）来安全地处理 Stripe 集成.这允许正确的 Webhook 处理,订阅管理和基于支付级别的基于角色的访问控制.
...
<Steps>
  <Step title="第 1 步:连接 superun 云服务">
    入门很简单. superun 使连接 superun 云服务 变得毫不费力:

1. 转到 **研发 → 服务**
    2. 找到 **superun 云服务** 并点击 **启用**
    3. 按照说明设置您的云项目
    4. 连接后,superun 云服务 可实现安全支付处理,订阅管理,Webhook 处理,客户数据存储和错误处理.
  </Step>

  <Step title="第 2 步:检索 Webhook 端点 URL">
    superun 生成 Webhook 处理程序后,从以下位置检索端点 URL:
    * **superun 云服务** → **函数** → 您的 webhook 函数
  ...
  <Step title="第 4 步:安全存储 Webhook 密钥">
    将 Webhook 签名密钥存储在您的 superun 云服务 环境变量中,或者在聊天中询问 superun 以帮助您安全地配置它.
...
<Accordion title="查看 superun 云服务 日志">
    <Steps>
        <Step title="第 1 步">
        转到 **研发 → 服务 → superun 云服务**
      </Step>
...
- 为订阅事件选择Webhook事件
- Webhook 端点正在正确处理事件
- Webhook 机密正确存储在 superun 云服务 中

<Card title="需要帮助吗?" icon="help-circle" href="/superun/introduction/faq">
